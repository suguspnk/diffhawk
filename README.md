...
PR where any configured identity is the requested reviewer, plus PRs already
tracked for that exact identity and repository, and the head commit hasn't
been reviewed by that identity yet. Tracked state is only a bounded fallback;
it never broadens discovery to untracked GitHub PRs. Within each account queue,
currently requested candidates are prioritized ahead of tracked fallback
candidates. The queue is round-robin across accounts and uses the configured
...
The request may be manual or generated by `CODEOWNERS`; the fallback never
expands discovery to an untracked PR.
