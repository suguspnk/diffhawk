...
const LAUNCHD_LABEL = 'ai.socialpost.openrevuwer.poll';
...
function schedulerStateHome(environment, homeDirectory, pathModule) {
  return environment.OPENREVUWER_HOME || pathModule.join(homeDirectory, '.openrevuwer');
}

function schedulerEnvironment(environment) {
  const persisted = {};
  if (environment.PATH) persisted.PATH = environment.PATH;
  if (environment.OPENREVUWER_HOME) {
    persisted.OPENREVUWER_HOME = environment.OPENREVUWER_HOME;
  }
  return persisted;
}
...
export function launchdPreview(options) {
  ...
  const command = scheduledCommand(options);
  const plistPath = path.join(homeDirectory, 'Library', 'LaunchAgents', `${LAUNCHD_LABEL}.plist`);
...
export async function installLaunchd({
  ...
  try {
    await execFileAsync(launchctlCommand, ['unload', plistPath]);
  } catch {
    // Not previously loaded — fine.
  }
  await execFileAsync(launchctlCommand, ['load', plistPath]);
}

export function schtasksPreview(options) {
  const { intervalMinutes } = options;
  const taskName = 'openrevuwer-poll';
...
export async function installSchtasks({
  ...
  await writeSchedulerEnvironment(options);
  await execFileAsync(schtasksCommand, args);
}
