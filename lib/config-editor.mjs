...
function repositoryCount(config) {
  return config.githubAccounts.reduce(
...
function accountSummary(config) {
  const accountCount = config.githubAccounts.length;
...
export async function persistConfigUpdate({
  ...
  const normalized = validateConfig(nextConfig);
...
async function consentedConfigUpdate(currentConfig, nextConfig) {
  ...
  const consent = await p.confirm({
    message:
      ...
      `repositories across ${nextConfig.githubAccounts.length} account(s)?`,
    initialValue: false,
  });
