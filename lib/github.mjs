...
export async function searchReviewRequestedPRs({ username, repo, auth }) {
  ...
  for (const line of out.split('\n').map((candidate) => candidate.trim()).filter(Boolean)) {
    if (line.startsWith('meta|')) {
      const [, totalCountText, incompleteResultsText] = line.split('|');
      const parsedTotalCount = Number(totalCountText);
      if (
        !Number.isSafeInteger(parsedTotalCount) ||
        (incompleteResultsText !== 'true' && incompleteResultsText !== 'false')
      ) {
        throw new Error('GitHub search returned malformed result metadata');
      }
      totalCount = Math.max(totalCount ?? 0, parsedTotalCount);
      incompleteResults ||= incompleteResultsText === 'true';
      continue;
    }
    ...
    if (!repoSlug || !Number.isSafeInteger(number) || number <= 0) {
  ...
  // Search stops exposing results after 1,000 matches even when --paginate is
  // used. The repository pull-request list endpoint has no equivalent search
  // window, so use it only for this degraded case and filter to the direct
  // user's requested_reviewers entry in jq. The stdout bound in ghSpawn still
  // fails closed if an unusually large repository exceeds the process limit.
  const fallbackOut = await gh([
    'api', '--paginate', '--method', 'GET',
    `/repos/${normalizedRepo}/pulls`,
    '-f', 'state=open', '-f', 'per_page=100',
    '--jq',
    `.[] | select(any(.requested_reviewers[]?; (.login // "") | ascii_downcase == ${JSON.stringify(normalizedUsername.toLowerCase())})) | (.number | tostring)`,
  ], { auth });
  const fallbackCandidates = fallbackOut
    .split('\n')
    .map((line) => line.trim())
    .filter(Boolean)
    .map((numberStr) => {
      ...
      if (!Number.isSafeInteger(number) || number <= 0) {
        throw new Error('GitHub pull request fallback returned a malformed candidate');
      }
      return { repo: normalizedRepo, number };
    });

  const candidates = new Map(
    searchCandidates.map((candidate) => [
      `${candidate.repo.toLowerCase()}#${candidate.number}`,
      candidate,
    ]),
  );
  for (const candidate of fallbackCandidates) {
    candidates.set(`${candidate.repo.toLowerCase()}#${candidate.number}`, candidate);
  }
...
async function reconcileSubmittedReview(options, originalError) {
  ...
  try {
    ...
    return await scheduleMutation(() => reviewAlreadyPosted(reviewOptions));
