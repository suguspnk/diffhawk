...
function ghSpawn(args, { input, timeoutMs = GH_TIMEOUT_MS, auth } = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      clearTimeout(timer);
      if (timedOut) {
        reject(Object.assign(new Error(`gh ${args.join(' ')} timed out after ${timeoutMs}ms`), { code: 'ETIMEDOUT' }));
      } else if (code !== 0) {
        reject(Object.assign(new Error(stderr || `gh ${args.join(' ')} exited ${code}`), { stderr }));
      } else {
        resolve(stdout);
      }
    });
...
export async function searchReviewRequestedPRs({ username, repo, global = false, auth }) {
  const query = global
  ...
  const out = await gh([
    'api', '--paginate', '--method', 'GET', '/search/issues', '-f', `q=${query}`,
