...
import { prepareCommand } from './process-launch.mjs';
...
export function buildPrompt({ template, learnings, pr, diff, focus, candidateFindings }) {
  ...
  // A user may replace the prompt with checklist-only content. Treating it
  // as a literal template would silently omit the diff and PR information,
  // so wrap it with the fixed framing when {{diff}} is absent.
...
export function invokeReviewer({
  ...
  const launch = prepare(cmd, args, { platform, environment }).catch((err) => {
    throw new Error(`failed to launch "${reviewerCommand}": ${err.message}`);
  });

  return launch.then((prepared) => new Promise((resolve, reject) => {
    let child;
    try {
