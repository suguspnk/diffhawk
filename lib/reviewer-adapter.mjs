...
function extractJson(text, accept = () => true) {
  ...
  // Parsing each balanced, schema-shaped span can still be expensive for
    ...
    for (let index = 0; index < candidate.length; index += 1) {
      ...
      if (frame?.inString) {
        ...
        if (char === '\\') {
          ...
          if (frame.stringValue === 'summary') frame.hasSummary = true;
          if (frame.stringValue === 'findings') frame.hasFindings = true;
        } else {
          frame.stringValue += char;
        }
        continue;
      }
      if (char === '"') {
        ...
        stack.pop();
        if (!frame.hasSummary || !frame.hasFindings) continue;
