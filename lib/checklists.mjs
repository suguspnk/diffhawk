...
export function checklistPathFor(repo) {
  ...
  // docs/checklists/<owner>/<name>.md avoids collapsing the owner/name
  ...
  // silently sharing (and cross-contaminating) their checklists — exactly
  // the bug per-repo checklists exist to prevent. Nesting by owner sidesteps
  // that ambiguity entirely rather than trying to pick an escape scheme.
  ...
  // never do.
  return `docs/checklists/${owner}/${name}.md`;
}
