...
function trackedCandidatesForRepository(state, account, repo) {
...
export async function pollOnce({
  ...
  async function discoverAccount(account) {
    ...
    for (const repo of account.repositories) {
      ...
      const trackedCandidates = trackedCandidatesForRepository(state, account, repo).map((candidate) => ({
          ...candidate,
          source: 'tracked',
        }));
      const candidateGroups = [
        ...(!searchFailed
          ? [{ source: 'requested', candidates: requestedCandidates }]
          : []),
        { source: 'tracked', candidates: trackedCandidates },
      ];
      candidates = candidateGroups.flatMap(({ source, candidates: group }) => {
        const rotated = rotateCandidateGroup(group, {
          account,
          repo,
          source,
          state,
        ...
        if (rotated.length > 0) {
          const cursor = rotated[0].candidateCursor;
          candidateCursorPlans.set(cursor.key, cursor);
        }
        return rotated;
      });
      if (!searchFailed && candidates.length === 0) {
  ...
  if (candidateMetadataDeferredCount > 0) {
    ...
    failures.push({
      status: 'failed',
      subject: 'review queue',
      note: `${candidateMetadataDeferredCount} candidate(s) deferred by metadata budget`,
    });
    await appendFailure(
      logPath,
      'candidate metadata',
