...
export async function pollOnce({
  ...
  async function discoverAccount(account) {
    ...
    for (const repo of account.repositories) {
      ...
      candidates = [
        ...(searchFailed ? [] : requestedCandidates),
        ...trackedCandidatesForRepository(state, account, repo).map((candidate) => ({
          ...candidate,
          source: 'tracked',
        })),
      ];
  ...
  let actionableReviewsStarted = 0;
  ...
  async function runReviewCandidate({
    ...
    if (actionableReviewsStarted >= MAX_REVIEWS_PER_POLL) {
