...
const MACOS_ALERTER_PATH = path.join(
  packageRoot,
  'vendor',
  'alerter',
);
...
function executeWithoutOutput(spawnImpl, command, args, options) {
  return new Promise((resolve, reject) => {
...
export async function deliverDesktopNotification(
  notification,
  {
    ...
    macAlerterPath = MACOS_ALERTER_PATH,
  ...
  if (platform === 'darwin') {
    if (Number.isFinite(darwinMajor) && darwinMajor >= 22) {
      ...
      if (notification.bypassFocus === true) {
        args.push('--ignore-dnd');
      }
