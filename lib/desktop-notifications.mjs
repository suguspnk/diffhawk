...
export function buildPollNotification({ outcomes = [], failures = [] } = {}) {
  ...
  const onlyDeferred = entries.every((entry) => entry.status === 'deferred');
  let title = 'OpenMergeLens review complete';
  if (attention) title = 'OpenMergeLens needs attention';
  else if (onlyDryRuns) title = 'OpenMergeLens dry run complete';
  else if (onlyRecoveries) title = 'OpenMergeLens tracking recovered';
  else if (onlyDeferred) title = 'OpenMergeLens review deferred';
...
export async function attemptDesktopNotification(
  ...
  try {
    ...
    try {
      ...
      // Notification logging is best-effort and must not replace the failure.
    }
    return false;
  }
}
