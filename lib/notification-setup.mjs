...
export function notificationSetupGuidance(platform = process.platform) {
  if (platform === 'darwin') {
    return (
      ...
      'notifications visible until dismissed, and temporarily turn off Focus ' +
      'or add OpenMergeLens to that Focus mode’s Allowed Apps. The locally ' +
      'signed helper cannot override Focus. On macOS 12 or earlier, check ' +
  ...
  if (platform === 'win32') {
    return (
      ...
      'allow notifications from OpenMergeLens or PowerShell. OpenMergeLens ' +
      'uses reminder toasts that stay visible until dismissed.'
    );
  }
  if (platform === 'linux') {
    return (
