...
export function sameReviewer(left, right) {
  return Boolean(left && right) && reviewerKey(left) === reviewerKey(right);
}
...
export function migrateLegacyStateForReviewer(
  state,
  previousReviewer,
  selectedReviewer,
) {
  if (!sameReviewer(previousReviewer, selectedReviewer)) return false;
  return migrateLegacyState(state, selectedReviewer);
}
