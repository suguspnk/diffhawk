...
function safePullRequestUrl(value, { hostname, repo, number }) {
  ...
  try {
    ...
    const expectedPath = `/${repo}/pull/${number}`;
    if (
      url.protocol !== 'https:' ||
      url.username ||
      url.password ||
...
function shouldShowAccounts(entries) {
  const accounts = new Set(entries.map((entry) => entry.account).filter(Boolean));
  const pullRequests = new Set();
  let duplicate = false;
  for (const entry of entries) {
    const key = `${entry.repo.toLowerCase()}#${entry.number}`;
    if (pullRequests.has(key)) duplicate = true;
    pullRequests.add(key);
  }
  return accounts.size > 1 || duplicate;
}
...
function renderEntry(entry, showAccount) {
  const statusClass = ATTENTION_STATUSES.has(entry.status)
    ...
    : entry.status === 'recovered'
      ...
      : '';
  const metadata = [
    showAccount && entry.account ? `as ${entry.account}` : '',
    entry.note,
  ].filter(Boolean);
    ...
    <article class="entry">
      <div class="entry-main">
        <div class="entry-topline">
          ...
          <span class="pr">${escapeHtml(entry.repo)}#${entry.number}</span>
        </div>
        <p class="title">${escapeHtml(entry.title || 'Untitled pull request')}</p>
...
<body>
  <main>
    ...
    <section class="report-list" aria-label="Pull request results">
      ${entries.map((entry) => renderEntry(entry, showAccount)).join('')}
    </section>
    <footer>Generated locally by OpenMergeLens · ${escapeHtml(entries.length)} pull request${entries.length === 1 ? '' : 's'}</footer>
  </main>
</body>
</html>
`;
...
<body>
  <main>
    ...
    <section class="report-list"><article class="entry"><div class="entry-main"><p class="title">The report was removed by OpenMergeLens’s retention policy.</p><p class="meta">Run <code>openmergelens report --list</code> to choose from the reports that are still available.</p></div></article></section>
  </main>
</body>
</html>
`;
...
async function atomicWrite(
  ...
  try {
    ...
    await enforcePrivateMode(targetPath, PRIVATE_FILE_MODE);
  } finally {
...
async function cleanPartialReportArtifacts(reportsDirectory, validIds) {
  ...
  await Promise.all(directoryEntries.map(async (entry) => {
    ...
    if (REPORT_TEMP_FILE_PATTERN.test(entry.name)) {
...
export async function createPollReport(
  ...
  const metadata = {
    ...
    firstPullRequest: `${entries[0].repo}#${entries[0].number}`,
  };
