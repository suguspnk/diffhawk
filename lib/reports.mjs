    ...
    .map((accountGroup, accountIndex) => {
      ...
      const repositories = accountGroup.repositories
        ...
        <section class="repository-group" aria-labelledby="${repositoryHeadingId}">
          <div class="repository-heading">
            <h3 class="repository-name" id="${repositoryHeadingId}">${escapeHtml(repositoryGroup.repo)}</h3>
            <span class="group-count">${escapeHtml(pullRequestCountLabel(repositoryGroup.entries.length))}</span>
          </div>
      ...
      <section class="account-group" aria-labelledby="${accountHeadingId}">
        <div class="account-heading">
          ...
          <span class="group-count">${escapeHtml(pullRequestCountLabel(accountGroup.entries.length))}</span>
        </div>
        ${repositories}
      </section>`;
    })
