...
const LOG_COORDINATION_ATTEMPTS = 200;
...
function redactSecretPrefixValues(value) {
  ...
  let cursor = 0;
  let match;
  while ((match = prefixPattern.exec(value))) {
    const valueStart = prefixPattern.lastIndex;
    const valueEnd = scanUnquotedCredentialValue(value, valueStart);
    if (valueEnd === valueStart) continue;
...
export function sanitizeDiagnostic(
  ...
  let sanitized = normalizeDiagnosticControls(stringifyDiagnostic(value))
    .replace(
