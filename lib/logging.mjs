...
function redactSecretPrefixValues(value, { controlSeparatedOnly = false } = {}) {
  ...
  while ((match = prefixPattern.exec(matchingValue))) {
    ...
    // `password\u000BHash=...`) is normalized later for key redaction. Only
    // treat it as a prefix separator when it introduces a quoted value.
    if (
      controlSeparatedOnly &&
      !['"', "'"].includes(matchingValue[valueStart])
    ) {
      continue;
    }
