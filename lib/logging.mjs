...
// Keep cross-process lock-election retries finite while giving a busy runner
// enough opportunities to observe the current owner after a probe timeout.
const LOG_COORDINATION_ATTEMPTS = 400;
const LOG_COORDINATION_RETRY_MS = 25;
const LOG_COORDINATION_PROBE_ATTEMPTS = 3;
const LOG_COORDINATION_PROBE_TIMEOUT_MS = 100;
...
function scanCookieHeaderValue(value, valueStart) {
  let quote;
  for (let index = valueStart; index < value.length; index += 1) {
    const character = value[index];
    if (quote) {
...
function redactCookiePairs(value, { setCookie = false } = {}) {
  let output = '';
  let cursor = 0;
  let segmentStart = 0;
  ...
  while (segmentStart < value.length) {
    ...
    for (let index = segmentStart; index < value.length; index += 1) {
      ...
      if (equalsIndex < 0 && character === '=') {
        equalsIndex = index;
      } else if (equalsIndex >= 0 && (character === '"' || character === "'")) {
        quote = character;
      } else if (character === ';' || /[\r\n\u2028\u2029]/u.test(character)) {
        segmentEnd = index;
        break;
    ...
    if (equalsIndex >= 0) {
      ...
      while (valueStart < segmentEnd && /[ \t]/u.test(value[valueStart])) {
        valueStart += 1;
      }
      let valueEnd = segmentEnd;
      while (valueEnd > valueStart && /[ \t]/u.test(value[valueEnd - 1])) {
        valueEnd -= 1;
      }
      output += value.slice(cursor, valueStart) + '[REDACTED]' + value.slice(valueEnd, segmentEnd);
      cursor = segmentEnd;
      if (setCookie) return output + value.slice(cursor);
    }
...
function redactCookieHeaderValues(value) {
  const keyUnit = '(?:[A-Za-z-]|\\\\u[0-9a-fA-F]{4})';
  const cookieKeyPattern = new RegExp(
    `(?<![A-Za-z0-9_-])(["']?)(${keyUnit}{1,${LOG_SECRET_KEY_MAX_PATTERN_CHARS}})\\1` +
      '([ \\t]*(?::|=)[ \\t]*)',
    'gi',
  );
  let output = '';
  let cursor = 0;
  let match;
  while ((match = cookieKeyPattern.exec(value))) {
    const [, , rawKey] = match;
    const key = normalizeCredentialKey(rawKey).toLowerCase();
    if (key !== 'cookie' && key !== 'set-cookie') continue;

    const valueStart = cookieKeyPattern.lastIndex;
    if (valueStart >= value.length) continue;
    const quote = value[valueStart];
    const valueEnd = quote === '"' || quote === "'"
      ? scanQuotedCredentialValue(value, valueStart)
      : scanCookieHeaderValue(value, valueStart);
    const contentStart = quote === '"' || quote === "'"
      ? valueStart + 1
      : valueStart;
    const contentEnd = quote === '"' || quote === "'"
      ? Math.max(contentStart, valueEnd - 1)
      : valueEnd;
    const redacted = redactCookiePairs(value.slice(contentStart, contentEnd), {
      setCookie: key === 'set-cookie',
    });
    if (redacted === value.slice(contentStart, contentEnd)) continue;

    output += value.slice(cursor, contentStart) + redacted;
    cursor = contentEnd;
    cookieKeyPattern.lastIndex = valueEnd;
  }
  return output + value.slice(cursor);
}
...
export function sanitizeDiagnostic(
  ...
  sanitized = redactCookieHeaderValues(sanitized);
...
async function appendToLogFile(logPath, line) {
  ...
  try {
    ...
    await enforcePrivateModeHandle(handle, PRIVATE_FILE_MODE);
  } finally {
    await handle.close();
  }
}
  ...
  for (let attempt = 0; attempt < LOG_COORDINATION_ATTEMPTS; attempt += 1) {
    try {
      const release = await acquireLock(lockPath, {
        probeAttempts: LOG_COORDINATION_PROBE_ATTEMPTS,
