...
function normalizeAccount(account) {
  ...
  if (!hostname || !username || /\s/.test(hostname) || /\s/.test(username)) {
    throw new Error('GitHub account must include a valid hostname and username');
  }
...
export function configuredGitHubAccount(config) {
  if (config?.githubAccount) return normalizeAccount(config.githubAccount);

  // Backwards compatibility for config files created before account pinning.
  if (config?.githubUsername) {
    return normalizeAccount({
      hostname: config.githubHostname || 'github.com',
      username: config.githubUsername,
    });
  }

  throw new Error('config.json is missing githubAccount (or legacy githubUsername)');
}

export function legacyGitHubAccount(config) {
  if (!config?.githubUsername || config.githubAccount) return null;
  return normalizeAccount({
    hostname: config.githubHostname || 'github.com',
    username: config.githubUsername,
  });
...
export async function resolveGitHubAuth(account) {
...
export function authEnvironment(auth, baseEnvironment = process.env) {
  ...
  const { hostname, username } = normalizeAccount(auth);
  const environment = {
    ...baseEnvironment,
    GH_HOST: hostname,
  ...
  environment.OPENREVUWER_GITHUB_ACCOUNT = `${username}@${hostname}`;
  return environment;
}
