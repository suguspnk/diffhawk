...
async function readOwnerMarkerContents(markerPath) {
  ...
  try {
    ...
    marker = await open(
      markerPath,
      ownerMarkerOpenFlags(constants.O_RDONLY),
    );
    stage = 'stat';
    const opened = await marker.stat();
    if (!opened.isFile()) return null;
    if (opened.dev !== existing.dev || opened.ino !== existing.ino) {
      if (process.env.OPENMERGELENS_DEBUG_LOCK === '1') {
        console.error('[openmergelens lock debug]', JSON.stringify({
          markerPath,
          stage: 'identity',
          existing: { dev: existing.dev, ino: existing.ino },
...
function ownerMarkerOpenFlags(access) {
  // Node only supports the basic access/creation flags on Windows. The
  // regular-file lstat guard remains in place there; POSIX-only flags are
  // retained on platforms that implement them.
  const platformFlags = process.platform === 'win32'
    ? 0
    : constants.O_NONBLOCK | (constants.O_NOFOLLOW ?? 0);
  return access | platformFlags;
}
...
async function writeOwnerMarker(markerPath, owner, nonce) {
  ...
  const flags = ownerMarkerOpenFlags(constants.O_WRONLY | constants.O_CREAT);
