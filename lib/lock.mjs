...
async function gateSnapshot(gatePath) {
  try {
    // Stat on both sides of the read so a replacement during inspection
    // cannot produce a snapshot that mixes two gate generations.
    const before = await stat(gatePath);
    const content = await readFile(gatePath, 'utf8');
    const after = await stat(gatePath);
    if (!sameFileGeneration(before, after)) return null;
...
function sameGateSnapshot(left, right) {
  return Boolean(left && right) &&
    sameFileGeneration(left.stat, right.stat) &&
  ...
  while (true) {
    ...
    try {
      ...
      if (err.code === 'EEXIST') {
        ...
        // finish.
        const observedGate = await gateSnapshot(gatePath);
        if (!observedGate) continue;
        if (Date.now() - observedGate.stat.mtimeMs <= GATE_STALE_MS) {
          // An active (or merely recent leaked) gate should not spin this
          // poll in a tight write/link/stat loop. Let a later scheduled poll
          // retry after the conservative stale interval.
          return null;
        }

        // Re-read immediately before removal and only unlink the exact gate
        // generation inspected above. This prevents a contender holding a
        // stale observation from deleting a newer gate created in the
        // meantime.
        const currentGate = await gateSnapshot(gatePath);
        if (!sameGateSnapshot(observedGate, currentGate)) return null;
        try {
