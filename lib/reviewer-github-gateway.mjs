      ...
      try {
        ...
        if (inspectionRequest.operation === 'metadata') {
          ...
          try {
            ...
            if (responseSent || response.destroyed) return;
            const cursor = inspectionRequest.cursor ?? 0;
            ...
            if (
              admission.servedCursors.has(cursor) &&
              !sharedFetchWasPending
            ) {
              const error = new Error(
            ...
            if (fileContextReads >= MAX_FILE_CONTEXT_READS) {
              const error = new Error(
            ...
            if (fileContextOutputBytes + pageOutputBytes > fileContextOutputBudgetBytes) {
              const error = new Error(
