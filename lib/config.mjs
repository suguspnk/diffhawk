...
function migrateLegacyConfig(input) {
  if (input.configVersion !== LEGACY_CONFIG_VERSION) return input;
  ...
  let aiProcessingConsent = false;
  if (Array.isArray(input.githubAccounts) && input.githubAccounts.length > 0) {
    aiProcessingConsent = input.githubAccounts.every((account) => {
      if (
...
export function validateConfig(input) {
  ...
  input = migrateLegacyConfig(input);
  ...
  if (
    input.aiProcessingConsent !== undefined &&
    typeof input.aiProcessingConsent !== 'boolean'
  ) {
    throw new Error('config.json aiProcessingConsent must be true or false');
  }
  ...
  return {
    configVersion: CONFIG_VERSION,
    githubAccounts,
    aiProcessingConsent: input.aiProcessingConsent === true,
