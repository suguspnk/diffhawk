...
import { upgradeReviewerCommand } from './reviewer-command-defaults.mjs';
...
const ACCOUNT_FIELDS = new Set(['hostname', 'username', 'repositories']);
...
export function validateConfig(input) {
  ...
  if (input.configVersion !== CONFIG_VERSION) {
    throw new Error(
      `config.json must use configVersion ${CONFIG_VERSION}; run \`openrevuwer init\``,
    );
  }
  ...
  return {
    ...
    githubAccounts,
    reviewerCommand: upgradeReviewerCommand(input.reviewerCommand),
