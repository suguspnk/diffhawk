...
import { validateReviewerCommandContract } from './reviewer-command-defaults.mjs';
import {
  enforcePrivateMode,
...
export function validateConfig(input) {
  ...
  const githubAccounts = input.githubAccounts.map((candidate, accountIndex) => {
    ...
    if (
      candidate.aiProcessingConsent !== undefined &&
      !Array.isArray(candidate.aiProcessingConsent)
    ) {
      throw new Error(
        `config.json githubAccounts[${accountIndex}].aiProcessingConsent ` +
        'must be an array of selected repositories',
      );
    }
    const seenConsents = new Set();
    const aiProcessingConsent = (candidate.aiProcessingConsent || []).map(
      (repository) => {
        const normalized = normalizeRepository(repository);
        const repositoryKey = normalized.toLowerCase();
        if (!repositoryKeys.has(repositoryKey)) {
          throw new Error(
            `config.json account ${accountLabel(account)} has AI-processing consent ` +
            `for unselected repository ${normalized}`,
          );
        }
        if (seenConsents.has(repositoryKey)) {
          throw new Error(
            `config.json account ${accountLabel(account)} contains duplicate ` +
            `AI-processing consent for ${normalized}`,
          );
        }
        seenConsents.add(repositoryKey);
        return repositories.find(
          (selected) => selected.toLowerCase() === repositoryKey,
        );
      },
    );

    return { ...account, repositories, aiProcessingConsent };
  });

  return {
    configVersion: CONFIG_VERSION,
    githubAccounts,
    reviewerCommand: validateReviewerCommandContract(input.reviewerCommand),
