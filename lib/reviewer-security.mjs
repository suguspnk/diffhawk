...
const GITHUB_CREDENTIAL_KEYS = new Set([
  'GH_ENTERPRISE_TOKEN',
  'GH_TOKEN',
...
const GITHUB_GATEWAY_KEYS = new Set([
  ...GITHUB_REVIEW_KEYS,
...
export function withoutGitHubCredentials(environment) {
  ...
  for (const key of GITHUB_GATEWAY_KEYS) {
...
function commandName(command) {
  // reviewerCommand is a full, portable command string for generated
  // backends (for example, `codex exec --ephemeral ...`), not just the
  // executable name. Inspect only its first token so backend-specific auth
  // variables are retained while custom commands remain restricted to the
  // common environment allowlist.
