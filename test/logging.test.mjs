...
test('keeps a near-cap active log bounded across concurrent processes', async (t) => {
  ...
  const loggingModulePath = pathToFileURL(path.resolve('lib/logging.mjs')).href;
  const childCode = [
  ...
  const childCount = process.platform === 'win32' ? 16 : 200;
  ...
  // there. One sixteen-child wave still exercises cross-process rotation and
