...
- **Each `lib/*.mjs` module owns exactly one external boundary.**
  ...
  its job from elsewhere — this is what keeps each boundary independently
...
- **Default to Node built-ins over adding a package.** `node:child_process`,
  ...
  that already shells out and handles GitHub credentials — that's a real
...
- **When a boundary genuinely needs mocking to test (subprocess calls,
  ...
  but it's the pattern to reach for once one does — e.g. testing
